...
const TrackParcel = () => {
  ...
  const currentStatus = parcel?.checkpoints?.length > 0 ? parcel.checkpoints[parcel.checkpoints.length - 1].status : "arrived"
    ...
    <>
      <div className="min-h-screen pt-24 pb-20">
        <div className="container mx-auto px-4 md:px-6">
          ...
          >
            <Input placeholder="Enter Tracking ID (e.g. IND-CRR-123456)"
              value={trackingId}
              onChange={(e) => setTrackingId(e.target.value)}
              onKeyDown={(e) => e.key === "Enter" && handleTrack()}
            />

            <Button onClick={handleTrack} disabled={loading} className="bg-primary text-primary-foreground shrink-0">
